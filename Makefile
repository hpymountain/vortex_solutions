...
virtualenv:
	virtualenv -q --prompt '> winterfell <' .venv
...
doc:
	rm -rf html
	pdoc3 --html winterfell tests
	open http://localhost:9999
	pdoc3 --html --http localhost:9999 winterfell tests
...
fmt:
	...
	pyink --pyink-use-majority-quotes --line-length 115 --include "\.py$"" --exclude "\/__pycache__\/" winterfell tests docs setup.py
